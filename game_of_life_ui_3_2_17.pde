...
void draw(){
  ...
  printMatrix();
  fill(255,0,0);
  //text(mouseX,40 + width / 2,height / 2);
  //text(HEIGHT,20 + width / 2,20 + height / 2);
  //text(numRows,width / 2,40 + height / 2);
}
int[][] assessMatrix(){
...
void printMatrix(){
  //THIS FUNCTION TAKES THE CURRENT VERSION OF INITIAL ARRAY AND DISPLAYS IT
  for(int i = 0; i < numRows; i++){
    for(int j = 0; j < numCols; j++){
...
void cursorControlMatrix(){
  ...
  */
  ...
  int cursorCol = mouseX * (numCols/WIDTH);
  int cursorRow = mouseY/HEIGHT * numRows;
  //CHANGE THAT CELL DEPENDING ON mousePressed
  if(initialMatrix[cursorRow][cursorCol] == 0 && mousePressed){
    initialMatrix[cursorRow][cursorCol] = 1;
  }else if(initialMatrix[cursorRow][cursorCol] == 1 && mousePressed){
    initialMatrix[cursorRow][cursorCol] = 0;
  }
}