...
void recolorMatrix(){
    for(int i = 0; i < numRows; i++){
      for(int j = 0; j < numCols; j++){
        int matrixValue = (int)random(0,1.5);
        initialMatrix[i][j] = matrixValue;
        if(identifyNeighborsStaticBlock() && previousStaticBlock){
          colorNeighborsStaticBlock();
        }
        previousStaticBlock = identifyNeighborsStatiBlock();
      }
    }
...
void colorNeighborsStaticBlock(){
      //origin
...
boolean identifyNeighborsStaticBlock(){
